...
import (
	...
	"github.com/bocdagla/clonehnews/hn"
)
...
const EXTRA_STORY_AMMOUNT int = 5 / 4
...
func getTopStories(numStories int) ([]item, error) {
	...
	for len(stories) < numStories {
		need := (numStories - len(stories)) * EXTRA_STORY_AMMOUNT
...
func getStories(ids []int) []item {
	...
	for i := 0; i < len(ids); i++ {
		go func(i, id int) {
			...
			resultCh <- result{idx: i, item: parseHNItem(item)}
		}(i, ids[i])
	}
